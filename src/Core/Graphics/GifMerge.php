<?php
/**
* Inevitable modification Log
* 1) Syntax error above php7
*    #PATCH001 PHP Fatal error:  Array and string offset access syntax with curly braces is no longer supported
* 2) Renamed constructor function for php7 and above
*    #PATCH002
* 3) To prevent conflicts with other modified versions
*    #PATCH003
* 4) Handling of large videos
*    #PATCH004
* 5) Change all tabs to soft space
* 6) For Composer support
*    Rename the file
*/
namespace Rindow\RL\Gym\Core\Graphics; #PATCH003
...
class GifMerge
{
    ...
    */
    ...
    :::::::::::::::::::::::::::::::::::::::::::::::
    */
    function __construct($images, $t1, $t2, $t3, $loop, $dl, $xpos, $ypos, $model, $stream=null ,$debug = false) #PATCH002
    {
    ...
    */
    function read_image_descriptor()
    {
        ...
        {
            $this->first = false;
            $this->putstring("\x47\x49\x46\x38\x39\x61"); // <= 'GIF89a' #PATCH004
    ...
    */
    ...
    function arrcmp($b, $s, $l)
    {
        for($i = 0; $i < $l; $i++)
        {
            if($s[$i] != $b[$i]) return false;  #PATCH001
        }
        return true;
    }
    ...
    {
        if($this->stream) {                               #PATCH004
            $bin = '';
            for($i = 0; $i < $l; $i++)                    #PATCH004
            {                                             #PATCH004
                $bin .= pack('C*', $s[$i]);               #PATCH004
            }                                             #PATCH004
            fwrite($this->stream,$bin);                   #PATCH004
        } else {                                          #PATCH004
            for($i = 0; $i < $l; $i++)
            {
    ...
    function putstring($s)                                #PATCH004
    {                                                     #PATCH004
        $s = array_map('ord',str_split($s));              #PATCH004
        $this->putbytes($s, count($s));                   #PATCH004
    }                                                     #PATCH004
